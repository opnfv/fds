...
Create sec groups
    ...
    Set Suite Variable  ${sec_gr_1}   ${result}
    ${result} =    Create security group no default rules    ${sg_client}
    Set Suite Variable  ${sec_gr_2}   ${result}

Create sec rules
    create security rule  ${sg_client}  egress  ipv4
    create security rule  ${sg_client}  ingress  ipv4
    create security rule  ${sg_server}  ingress  ipv4
    create security rule  ${sg_client}  egress  ipv4  proto=icmp
...
Modify policy
    ...
    create security rule  ${sg_client}  ingress  ipv4
    create security rule  ${sg_server}  ingress  ipv4
    create security rule  ${sg_client}  egress  ipv4  proto=tcp  port_min=80  port_max=80
...
Setup Suite
    ...
    Set Suite Variable  ${sec_gr_1}       ${None}
    Set Suite Variable  ${sec_gr_2}       ${None}
...
Teardown Suite
    ...
    Run Keyword If  $sec_gr_1 is not $None      delete security group  ${sec_gr_1}
    Run Keyword If  $sec_gr_2 is not $None      delete security group  ${sec_gr_2}
...
Create security group no default rules    ${name}
