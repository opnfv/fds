                ...
                if set_node.hasAttribute('name'):
                    if set_node.getAttribute('name') == 'host':
                        ...
                        if text == '':
                            ...
                            ports.append(property_node.getAttribute('default').strip())
                        else:
                            ports.append(text)
...
grep -Eo ': "\w+.*$' | grep -Eo '\w+'""".format(honeycomb_config_dir))[1].read().strip()
    ...
    def _poll_odl_rest(self, event=None):
        ...
        while datetime.datetime.now() - start < delta and event.is_set():
            try:
            ...
            raise ODLRestartedUnsuccessfully('ODL restart timeout out after {}'.format(self.odl_restart_timeout))
...
class StressTest(FDSLibrary):
    ...
    def __init__(self):
        ...
        self.routing_node = None
        self.background_check = threading.Event()
        self.background_check.set()
    ...
    def periodically_restart_odls(self):
        ...
        try:
            while self.background_check.is_set:
                self.odl_nodes[random.randint(0, odl_node_count - 1)].restart_odl(self.background_check)
    ...
    def verify_tenant_gateway_honeycomb(self):
        ...
        while datetime.datetime.now() - start < delta and self.background_check.is_set():
            if interface_list:
        ...
        try:
            ...
            try:
                while self._free_vm_space() and not self.odl_restart_error.is_set():
                    ...
                    non_iteration_threads = threading.enumerate()
                    ...
                    iteration_threads = set(threading.enumerate()) - set(non_iteration_threads)
                    for t in iteration_threads:
                        t.join(self.bridge_domain_verify_timeout + 5)

                    self.logger.debug('Number of alive threads {}'.format(threading.active_count()))
                    self.logger.info('Iteration for network {} done after {}'
                                     .format(self.current_network_id, datetime.datetime.now() - iteration_start))
                    self.logger.info('=========================================='
                                     '==========================================')

                    exceptions = [t.err for t in iteration_threads if t.err is not None]
