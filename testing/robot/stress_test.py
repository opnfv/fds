...
class StressTest(FDSLibrary):
    def __init__(self):
        ...
        self.current_compute_host = None
    ...
    def _create_port(self):
        ...
        logging.info("Creating port " + port_name)
    ...
    def _verify_bridge_domains(self):
        self.current_compute_host = None
        while self.current_compute_host is None:
            self.current_compute_host = getattr(self.nova_client.servers.get(self.current_vm_id),
                                                'OS-EXT-SRV-ATTR:host', None)
        bridge_domains = self._get_bridge_domains(self.current_compute_host)
        logging.info("Checking bridge domains on " + self.current_compute_host)
        while len(self.bridge_domains[self.current_compute_host]) == len(bridge_domains):
            time.sleep(1)
            bridge_domains = self._get_bridge_domains(self.current_compute_host)
        self.bridge_domains[self.current_compute_host] = bridge_domains

    def _create_vm(self):
        vm_name = 'stress-vm-' + str(self.created_vms)
        logging.info("Creating vm " + vm_name)
        self.created_vms += 1
    ...
    def _create_network(self):
        ...
        logging.info("Creating network " + net_name)
        ...
        logging.info("Creating subnet " + subnet_name)
    ...
    def _create_router(self):
        ...
        logging.info("Creating router " + router_name)
        body = {"router": {
            "name": router_name
        }}
    ...
    def _attach_subnet_to_router(self):
        logging.info("Attaching subnet to router")
    ...
    def run_test(self):
        try:
            ...
            try:
                ...
                sys.stderr.write('Test failed because of {}\n'.format(e))
        except Exception:
            sys.stderr.write('Test setup failed!\n')
        finally:
            self.tear_down()
    ...
    def set_up(self):
        ...
        self._update_hypervisors()
        for hypervisor in self.hypervisors:
            self.bridge_domains[hypervisor.hypervisor_hostname] = self._get_bridge_domains(hypervisor)
        # TODO check image
        # TODO check flavor
    ...
    def _get_bridge_domains(self, hostaname):
        bridge_domains = requests.get('http://{}:8183/restconf/config/v3po:bridge-domains/'
                                      .format(hostaname),
                                      auth=('admin', 'admin')).json()['bridge-domains']
