...
class StressTest(FDSLibrary):
    ...
    def verify_tenant_gateway_honeycomb(self):
        self.logger.info('Checking tenant gateway port in honeycomb')
        ...
        while datetime.datetime.now() - start < delta:
            if interface_list:
                self.logger.debug('Tenant gateway port {} created in honeycomb on node {}'
            ...
            self.logger.error('Tenant gateway port {} has not been created in time'
                              .format(self.current_tenant_gateway_port_id))
            # raise Exception('Timed out when checking that tenant gateway port has been created')
            exit()
        self.logger.info('Tenant gateway port {} has been created on {} after {}'
                         .format(self.current_tenant_gateway_port_id,
                                 self.routing_node,
    ...
    def verify_port_honeycomb(self):
        self.logger.info('Checking port in honeycomb')
        start = datetime.datetime.now()
        ...
        while datetime.datetime.now() - start < delta:
            if interface_list:
                if not interface_found:
                    ...
                    self.logger.debug('Port {} created in honeycomb on node {}'
                    ...
                    if l2_json:
                        self.logger.debug('L2 configuration {} on port {} created in honeycomb on node {}'
                                          .format(l2_json, self.current_port_id,
                                                  self.current_compute_hypervisor.hypervisor_hostname))
                        break
            ...
            self.logger.error('Port {} has not been created in honeycomb in time'.format(self.current_port_id))
            # raise Exception('Timed out when checking that port has been created')
            exit()
        self.logger.info('Port {} has been created on {} after {}'
                         .format(self.current_port_id,
                                 self.current_compute_hypervisor.hypervisor_hostname,
                                 datetime.datetime.now() - start))
    ...
    def verify_port_odl(self):
        self.logger.info('Checking port in odl')
        # TODO finish this
        start = datetime.datetime.now()
        ...
        while datetime.datetime.now() - start < delta:
            if port_list:
                if not interface_found:
                    interface_found = True
                    self.logger.debug('Port {} created in odl'.format(self.current_port_id))
                else:
                    port_json = port_list[0]
                    l2_json = port_json.get('neutron-binding:vif-details')
                    if l2_json:
                        self.logger.debug('Vif-details on port {} created in odl'.format(self.current_port_id))
                        break

            port_list = self._get_odl_json('/restconf/config/neutron:neutron/ports/port/'
                                           + self.current_port_id, 'port')
        else:
            self.logger.error('Port {} has not been created in time'.format(self.current_port_id))
            # raise Exception('Timed out when checking that port has been created')
            exit()
        self.logger.info('Port {} has been created on {} after {}'
                         .format(self.current_port_id,
    ...
    def verify_bridge_domains(self):
        ...
        while datetime.datetime.now() - start < delta:
            ...
            self.logger.debug('Bridge domains on node {} after timeout: {}'
                              .format(self.current_compute_hypervisor.hypervisor_hostname, bridge_domains))
            self.logger.error('Bridge domain {} has not been created in time'.format(self.current_network_id))
        ...
        self.logger.info('Bridge domain has been created on {} after {}'
