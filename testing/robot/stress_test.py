...
class StressTest(FDSLibrary):
            ...
            if interface_list:
                ...
                if l2_json:
                    logger.debug('L2 configuration {} on tenant gateway port {} created '
                                 'in honeycomb on node {}'.format(l2_json,
    ...
    def verify_port_honeycomb(self):
        ...
        while datetime.datetime.now() - start < delta:
            if interface_list:
                ...
                if l2_json:
                    ...
                    if ports_bridge_domain:
                        logger.debug('Port {} added to bridge domain {} in honeycomb on node {}'
            ...
            exit()
        logger.info('Port has been created in honeycomb on {} after {}'
    ...
    def verify_port_odl(self):
        ...
        while datetime.datetime.now() - start < delta:
            ...
            logger.error('Port {} has not been created in time in odl neutron'.format(self.current_port_id))
            # raise Exception('Timed out when checking that port has been created')
            exit()
        logger.info('Port has been created in odl neutron after {}'
