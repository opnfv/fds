...
class StressTest(FDSLibrary):
    def __init__(self):
        ...
        self.odl_lock = threading.Lock()
        self.honeycomb_lock = threading.Lock()
    ...
    def verify_tenant_gateway_honeycomb(self):
        ...
        logger.debug('Interface-list: {}'.format(interface_list))
        ...
        while datetime.datetime.now() - start < delta:
            if interface_list:
                if not interface_found:
                    ...
                    if l2_json:
                        self.logger.debug('L2 configuration {} on tenant gateway port {} created '
