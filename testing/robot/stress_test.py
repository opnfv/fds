...
class OpenstackNode(object):
    def __init__(self, host_ip, http_session, ssh_user, ssh_key_path, odl_rest_creds=None,
        ...
        self._odl_restart_delta = datetime.timedelta(seconds=self.odl_restart_timeout)
...
grep -Eo ': "\w+.*$' | grep -Eo '\w+'""".format(honeycomb_config_dir))[1].read().strip()
    ...
    def _poll_odl_rest(self, event=None):
        ...
        while datetime.datetime.now() - start < self._odl_restart_delta and not event.is_set():
            try:
                time.sleep(1)
    ...
    def exec_ssh_command(self, command):
        self.ssh_client.connect(self.host_ip, username=self.ssh_user, key_filename=self.ssh_key_path)
        (_, response, _) = self.ssh_client.exec_command(command)
        self.ssh_client.close()
...
class Results(object):
    ...
    def add_to_current_iteration(self, dict_data):
        self.results[-1].update(dict_data)
...
class StressTest(FDSLibrary):
    ...
    def __init__(self):
        ...
        self.resource_verification_timeout = 300
        self._resource_verification_delta = datetime.timedelta(seconds=self.resource_verification_timeout)
        self.bridge_domain_verification_timeout = 1200
        self._bridge_domain_verification_delta = datetime.timedelta(seconds=self.bridge_domain_verification_timeout)
    ...
    def periodically_restart_odls(self):
        ...
        # restart odl
        # TODO add options to run in parallel, in parrallel with timeout (also random timeout)
        #                     or after each of n networks have been created
    ...
    def periodically_restart_networking_vpp(self):
        pass
    ...
    @threaded
    def _verify_port_nosdn(self):
        logger = logging.getLogger('StressTest.NoSDNPortCheck')
        logger.setLevel(self.logger_level)
        controller = self.controller_nodes[0]
        logger.info('Checking VM port in etcd on node {}'.format(controller.hostname))
        # TODO could this be run without sshing into controller
        # if etcdctl is installed on the host where the test is running?
        start = datetime.datetime.now()
        command_string = 'etcdctl --endpoint=http://{}:2379 ls -r | grep ports | grep {} | grep state' \
            .format(controller.host_ip, self.current_port_id)
        verification_output = controller.exec_ssh_command(command_string)
        while datetime.datetime.now() - start < self._resource_verification_delta \
                and not self.stop_background_check.is_set():
            if verification_output != '':
                delta = datetime.datetime.now() - start
                logger.debug('VM port created after {}'.format(delta))
                logger.info('VM port has been created on {} after {}'
                            .format(self.current_compute_node.hostname, delta))
                self.results.add_to_current_iteration({'vm_port_creation': delta})
                break
            else:
                verification_output = controller.exec_ssh_command(command_string)
        else:
            logger.error("VM port {} has not been created in time".format(self.current_port_id))
            # raise Exception('Timed out when checking that port has been created')

    @threaded
    def _verify_tenant_gateway_honeycomb(self):
        ...
        while datetime.datetime.now() - start < self._resource_verification_delta \
            ...
            if interface_list:
                ...
                if l2_json:
                    ...
                    logger.info(
                        'Tenant gateway port has been created and added to bridge domain in honeycomb on {} after {}'
                            .format(self.routing_node.hostname, delta))
                    self.results.add_to_current_iteration({'tenant_gateway_port_honeycomb_creation': delta})
                    break
    ...
    @threaded
    def _verify_port_honeycomb(self):
                ...
                if l2_json:
                    ...
                    if ports_bridge_domain:
                        ...
                        logger.info('VM port has been created and added to bridge domain in honeycomb on {} after {}'
                                    .format(self.current_compute_node.hostname, delta))
                        self.results.add_to_current_iteration({'vm_port_honeycomb_creation': delta})
                        break
            elif interface_found:
            ...
            if port_list:
                ...
                if l2_json:
                    ...
                    logger.debug('Vif-details on VM port {} created in odl neutron after {}'
                                 .format(self.current_port_id,
                                         datetime.datetime.now() - interface_found))
                    logger.info('VM port has been created and has vif-details in odl neutron after {}'.format(delta))
                    self.results.add_to_current_iteration({'vm_port_odl_creation': delta})
                    break
    ...
    @threaded
    def _verify_bridge_domains_honeycomb(self):
        start = datetime.datetime.now()
        ...
        while datetime.datetime.now() - start < self._bridge_domain_verification_delta \
                and not self.stop_background_check.is_set():
    ...
    def _create_port(self):
        ...
        if 'odl' in self.scenario:
            self._verification_threads.append(self._verify_port_odl())
            self._verification_threads.append(self._verify_port_honeycomb())
            self._verification_threads.append(self._verify_bridge_domains_honeycomb())
        elif 'nosdn' in self.scenario:
            self._verification_threads.append(self._verify_port_nosdn())
    ...
    def _create_router(self):
        router_name = 'stress-router'
        self.logger.info('Creating router ' + router_name)
        body = {'router': {
            'name': router_name
        }}
        self.router_id = self.neutron_client.create_router(body=body)['router']['id']
        self.logger.debug('Created router with id ' + self.router_id)
        self.cleanup['routers'].append(self.router_id)

    def _attach_subnet_to_router(self):
        self.logger.info('Attaching subnet to router')
        body = {'subnet_id': self.current_subnet_id}
        self.current_tenant_gateway_port_id = self.neutron_client.add_interface_router(self.router_id,
                                                                                       body=body)['port_id']
        if 'odl' in self.scenario:
            self._verification_threads.append(self._verify_tenant_gateway_honeycomb())

    def report(self):
        self.results = {'duration': self.test_start - self.test_stop,
                        'networks_created': len(self.results),
                        'iterations': self.results}
        results_filename = 'stress_results.out'
        i = 0
        while os.path.isfile(results_filename):
            i += 1
            results_filename = 'stress_results{}.out'.format(i)
        with open(results_filename, mode='rw') as rfile:
            rfile.write(pprint.pformat(self.results))
        ...
        try:
            ...
            try:
                if 'odl' in self.scenario:
                    ...
                    self.periodically_restart_networking_vpp()
                while self._free_vm_space() and not self.odl_restart_error.is_set():
                    ...
                    for t in self._verification_threads:
                        t.join(self.bridge_domain_verification_timeout + 5)
                    ...
                    exceptions = [t.err for t in self._verification_threads if t.err is not None]
