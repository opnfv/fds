...
class StressTest(FDSLibrary):
    def __init__(self):
        ...
        self.resource_verify_timeout = 150
    ...
    def verify_tenant_gateway_honeycomb(self):
        ...
        while datetime.datetime.now() - start < delta:
            ...
            logger.error("Tenant gateway port {} has not been created "
                         "or hasn't been added to bridge domain in honeycomb in time"
                         .format(self.current_tenant_gateway_port_id))
    ...
    def verify_port_honeycomb(self):
        ...
        while datetime.datetime.now() - start < delta:
            ...
            logger.error("Port {} has not been created or hasn't been added to bridge domain in honeycomb in time"
                         .format(self.current_port_id))
    ...
    def verify_port_odl(self):
        ...
        while datetime.datetime.now() - start < delta:
            ...
            logger.error("Port {} has not been created or doesn't have vif-details in time in odl neutron"
                         .format(self.current_port_id))
            # raise Exception('Timed out when checking that port has been created')
            exit()
    ...
    def _get_odl_json(self, url, key):
        response = self.rest_get(self.odl_ip, self.odl_port, url, self.odl_auth, key)
        return response

    def _get_honeycomb_json(self, hostname, url, key):
        response = self.rest_get(hostname, self.honeycomb_port, url, self.honeycomb_auth, key)
        return response
