...
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null /root/.ssh/id_rsa.pub \
    root@$UNDERCLOUD_IP:/home/stack/id_rsa_jumphost_root.pub 2> /dev/null
...
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $SCRIPT_DIR/copy_keys.sh \
    root@$UNDERCLOUD_IP:/home/stack 2> /dev/null
...
do
    ...
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null stack@$UNDERCLOUD_IP \
        "/home/stack/copy_keys.sh $node_ip &" 2> /dev/null
    ...
    ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $node_name \
        'git clone https://gerrit.opnfv.org/gerrit/p/fds.git $overcloud_fds_repo_loc' 2> /dev/null
    if [[ $(ssh -oStrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root$node_ip \
        'grep -c ". /root/overcloudrc" /root/.bashrc') -eq 0 ]]
    then
        ...
        'echo ". /root/overcloudrc" >> /root/.bashrc'
    fi
...
echo
echo "Post-apex script is finished. If you want the nova and neutron debug configurations to take effect, \
you'll need to restart the services manually."
